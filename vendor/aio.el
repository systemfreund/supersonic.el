...
;; Vendored into subsonic.el's `vendor/` directory so `make compile`/`make
;; test` work offline without a package manager (see Makefile).  Sourced
;; from MELPA's `aio' package, version 1.1.  Bump this copy whenever
;; MELPA's `aio.el' changes in a way this project depends on.
